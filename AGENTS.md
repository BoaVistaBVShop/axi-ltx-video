...
- Por decisão explícita do usuário, o pacote da imagem no GHCR deve permanecer **privado**. A Runpod deverá usar autenticação de container registry vinculada ao template; não tornar o pacote público e não reutilizar credenciais S3, `RUNPOD_API_KEY` ou `HF_TOKEN` para essa finalidade.
...
- configurar e testar na Runpod a autenticação de container registry para obter a imagem GHCR privada; a decisão de manter o pacote privado já está tomada, mas a credencial específica ainda não foi criada/configurada e nunca deve ser registrada neste repositório;
