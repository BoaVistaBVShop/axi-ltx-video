...
O commit fixado `15d09abb5a18` do `ComfyUI-LTXVideo` é incompatível com Kornia 0.8.3 sem o reparo do upstream que remove o import de `pad` e usa `torch.nn.functional.pad`. O reparo idempotente está versionado em `scripts/pod/repair_ltx_kornia.py`, incorporado ao `Dockerfile` e publicado nas imagens vigentes desde `v0.4.0`. As versões anteriores permanecem apenas como histórico e não devem ser usadas em novas sessões.
...
- O template `axi-ltx-video-v0.5.0` (`6213ek6yok`) foi verificado com a imagem pública pelo digest imutável vigente, container disk de 150 GB, montagem em `/workspace`, somente `22/tcp`, sem autenticação de registry e com referência ao secret existente pela variável `HF_TOKEN`. Nenhuma porta HTTP ou de Jupyter foi publicada.
...
- O repositório público é `https://github.com/BoaVistaBVShop/axi-ltx-video`. Os builds `v0.1.0` a `v0.4.0` permanecem como histórico. A imagem vigente `v0.5.0`, construída pelo GitHub Actions a partir do commit `bd702b4d4bb131cecb993557cbb45140bb1dbd02`, concluiu com sucesso no run `33580818737` e tem digest imutável `sha256:e3c7a4985ceeea7dd9c4d9faf211dcd412f8fbad5edf53a06eb2deeec9b5e702`; ela contém os manifests, perfis A/B Axi360, downloaders, `/opt/ltx-stack/bootstrap.py` e o reparo LTX/Kornia.
...
- Dockerfile, build no GitHub Actions e imagem vigente `ghcr.io/boavistabvshop/axi-ltx-video:v0.5.0` publicada no digest imutável registrado em `config/stack.json`; versões anteriores permanecem apenas como histórico;
...
- template `axi-ltx-video-v0.5.0` (`6213ek6yok`) provisionado e verificado com imagem fixada por digest, somente SSH em `22/tcp`, montagem em `/workspace` e secret referenciado sem expor o valor;
